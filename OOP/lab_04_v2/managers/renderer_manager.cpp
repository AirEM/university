...
void RendererManager::setDrawer(std::shared_ptr<BaseDrawer> drawer) {
    _drawer = drawer;
}

void RendererManager::setCamera(std::shared_ptr<Camera> camera) {
    _camera = camera;
}

void RendererManager::draw(std::shared_ptr<Scene> scene)
{
    if (!_drawer || !_camera) {
...
void RendererManager::proectPoint(math::Point &p) {
